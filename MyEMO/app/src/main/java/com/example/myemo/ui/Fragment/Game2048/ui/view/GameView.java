...
import com.example.myemo.ui.Fragment.Game2048.game2048.MainActivity;
import com.example.myemo.ui.Fragment.Game2048.ui.view.Card;
...
public class GameView extends GridLayout { // 2048游戏主类 让这个类绑定xml
    ...
    public void addReButton(){
        ...
        re.setHeight(20);
        re.setText("Restart!");
        re.setBackgroundColor(Color.parseColor("#EECFA1"));
    ...
    private void initGameView() { // 初始化
        ...
        setRowCount(5);
        setBackgroundColor(Color.parseColor("#FFE4C4")); // 配置GameView的背景或颜色
...
//        addCards(cardWidth, cardHeight);
    ...
    private void checkComplete() {
        ...
        All: for (int y = 0; y < 4; y++) {
            for (int x = 0; x < 4; x++) {
                //若玩家合成了2048或者更大的数字 弹出提示框
                if(cardsMap[x][y].getNum() >= maxnum){
                ...
                if (cardsMap[x][y].getNum() == 0
                    ...
                    complete = false;
//                    break All; // 写一个标签，跳出所有循环   // 为了检测所有数字 不需要跳出循环
                }//判断结束的简略方法如下，基本无效
//                if(cardsMap[x][y].getNum()==0){
//                    complete = false;
//                    break All;
//                }
        ...
        // 弹出提示框
       Log.v("maxnum",maxnum+"");
        if(maxnum>=1024&&!complete){
           if(flags.get(maxnum)==0) {
               LoadingDialog.LDialog(MainActivity.getMainActivity(), maxnum);
               flags.put(maxnum,1);
           }
        }
        //  complete == true 表示已满 结束游戏
        if (complete) {
            LoadingDialog.LDialog(MainActivity.getMainActivity(),0);
