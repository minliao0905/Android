...
public class LoginActivity extends BaseActivity {
    ...
    private boolean isPsd = true;//是否是密码 。。#登录和使用快捷登录即手机号码，发送验证码登录
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.container:
                hideSoftKeyBoard();//#点击页面和按钮即隐藏软键盘
                break;
            case R.id.btLogin:
                hideSoftKeyBoard();
                if (isPsd) {
    ...
    public void login(String account, String psd) {
        ...
        showLoadingView();//#显示加载页面
        new Handler().postDelayed(() -> {
            dismissLoadingView();
            btLogin.setEnabled(true);//#重新设置btn监听
            if (isPsd) {
                if (dataManager.checkAccount(account, psd))   // #验证用户信息成功
